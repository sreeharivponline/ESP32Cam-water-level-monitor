...
firebase_url = "https://fir-test-379d7-default-rtdb.firebaseio.com/camera/image.json"

# Load the YOLOv8 model (assuming you have the pre-trained model or a custom one)
...
def fetch_and_show_image():
    ...
    if response.status_code == 200:
        ...
        if base64_image:
            ...
            annotated_img = results[0].plot()  # Plots the detections on the image

            # Display the annotated image
            cv2.imshow('Real-Time Camera Feed with YOLOv8', annotated_img)
...
while True:
    if not fetch_and_show_image():
        break
    time.sleep(0.01)  # Adjust the sleep duration for your desired FPS
